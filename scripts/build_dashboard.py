"""
Build the Inbound Call Service-Queue dashboard.

Source: data/ameyo_daily.csv (pulled from Metabase:
        PROD_DB.PUBLIC.AMEYO_CALL_DETAILS_REPORT,
        QUEUE_NAME in (high_pain_queue, low_pain_queue),
        CALL_TYPE = inbound.call.dial)

Output: output/inbound_call_dashboard.xlsx
...
OUT = _BASE / "output" / "inbound_call_dashboard.xlsx"
...
# Explicit order for middle classes. "Incomplete Call" is placed immediately
# after "Wiom Net" per request.  Any new class that appears in the data but
# is NOT in this list is inserted alphabetically at the end of the middle
# block, just before the pinned-bottom classes.
MIDDLE_ORDER = [
    "Change Request",
    "Partner Misbehavior",
    "Refund",
...
def threshold_for(value):
    """
    Upward-spike threshold, based on the observed value:
        value >= 100       -> 15%
        20 <= value < 100  -> 25%
        value < 20         -> 100%
    """
...
def spike_flag(value, baseline):
    """
    Return (is_spike, pct_increase) where pct_increase is integer %.
    Upward spikes only. Uses baseline as the comparison value.
    """
    ...
    if pct <= 0:
        return (False, 0)
    # threshold is decided by the larger of value and baseline (use value — the observed number)
    return (pct >= threshold_for(value), round(pct * 100))


def write_cell(ws, r, c, value, baseline, is_mtd_col=False, is_class=False):
    """
    Write a numeric cell.  The absolute number is always black; if the value
    exceeds its baseline by the threshold, a red "(+NN%) ↑" suffix is appended
    as rich text.  MTD-column numbers are bold.
    """
    ...
    # Rich text: black number, red suffix
...
def write_month_sheet(ym):
    ...
    for cls, codes in taxonomy:
        # Sum across ALL rows for this display-class (works for both
        # collapsed classes — where the only key is COLLAPSED_SENTINEL —
        # and expanded classes with many codes).
    ...
    # TOTAL row (all classes, all codes)
    ...
    ws.freeze_panes = "B2"  # freeze top header row + first column (Disposition)

    # footnote
    note_row = r + 3
    notes = [
        f"Filters: Queue = High_Pain_Queue + Low_Pain_Queue   |   "
...
# chart
chart = LineChart()
chart.title = "Month-on-Month Avg Daily Inbound Calls by Disposition Class"
chart.style = 12
...
lines = [
    ("Daily Inbound Call Summary — Service Queue Dashboard", True, 14, PINK),
    ("", False, 11, BLACK),
    ("Source", True, 12, BLACK),
    ("  Metabase (db 113 / Snowflake): PROD_DB.PUBLIC.AMEYO_CALL_DETAILS_REPORT", False, 11, BLACK),
    ("  Filters: QUEUE_NAME IN ('high_pain_queue','low_pain_queue')  AND  CALL_TYPE = 'inbound.call.dial'", False, 11, BLACK),
    ...
    ("  1. python scripts/pull_ameyo.py       # re-pulls data from Metabase", False, 11, BLACK),
    ("  2. python scripts/build_dashboard.py  # rebuilds this workbook", False, 11, BLACK),
    ("  New Disposition Codes / Classes (and new months) appear automatically on the next refresh.", False, 11, BLACK),
    ("  A daily Windows Scheduled Task runs 'refresh_dashboard.bat' to do both steps automatically.", False, 11, BLACK),
]
